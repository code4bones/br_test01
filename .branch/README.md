# Carry the Ribbon

B.R.A.N.C.H. — Blue Ribbon Autonomous Network for Carrier Hopping

Independent technical tribute to the Blue Ribbon Online Free Speech Campaign.
This repository does not imply affiliation with or endorsement by the Electronic
Frontier Foundation.

The Blue Ribbon is alive again.
From symbol to protocol.
The ribbon no longer merely hangs on the Web. It becomes a route through it.

Search locator: branchbootstrapv0
Search markers: branchbootstrapv0 BRANCH0 branch/connectivity/0 branch-bootstrap-v0 carry-the-ribbon

This bundle was generated from live signed bootstrap.beacon wrappers accepted by the local tool.

Optional root README snippet. Insert it manually into an existing repository
README only if you want the badge and public locator visible there. This bundle
does not create or replace a root README.md file.

```md
[![B.R.A.N.C.H. branchbootstrapv0 Blue Ribbon - Carry the Ribbon](.branch/ribbon.svg)](.branch/README.md)

B.R.A.N.C.H. bootstrap carrier: branchbootstrapv0 BRANCH0 branch/connectivity/0 branch-bootstrap-v0 carry-the-ribbon
```

Suggested GitHub repository description:

```text
B.R.A.N.C.H. bootstrap carrier branchbootstrapv0 carry-the-ribbon
```

Suggested GitHub topics:

```text
branchbootstrapv0, carry-the-ribbon, branch-protocol
```

The signed records are stored in `.branch/records.br0`. Repository ownership,
badges, topics, branch names, and CI status are publication evidence only. The
decoded B.R.A.N.C.H. signed event envelope remains the authority.
